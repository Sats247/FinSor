...
window.manageClearance = async function(imo) {
  ...
  const healthBadge = `<span class="badge" style="background:${isHC ? 'var(--color-success)' : 'var(--color-alert)'};color:#fff;">Health ${isHC ? '✓ Granted' : '✗ Denied'}</span>`;
  const customsBadge = `<span class="badge" style="background:${isCC ? 'var(--color-success)' : 'var(--color-alert)'};color:#fff;">Customs ${isCC ? '✓ Granted' : '✗ Denied'}</span>`;
  ...
  const drawerHTML = `
    ...
    <div style="margin-bottom:20px;padding:16px;border:1px solid var(--color-border);border-radius:8px;background:var(--color-surface)">
      ...
      ${isHC ? `<p style="font-size:12px;margin:0 0 12px;color:var(--color-text-secondary)">Granted by ${vessel.health_clearance_by || 'System'} at ${vessel.health_clearance_at ? formatDateTime(vessel.health_clearance_at) : 'Registration'}</p>` : ''}
      <div style="display:flex;gap:8px;margin-top:8px">
        ${isHC ? 
          `<button class="btn btn-danger btn-sm" onclick="submitClearance('${imo}', 'health', false)">Deny</button>` : 
          `<button class="btn btn-success btn-sm" onclick="submitClearance('${imo}', 'health', true)">Restore Grant</button>`
        }
      </div>
    </div>
    ...
    <div style="margin-bottom:20px;padding:16px;border:1px solid var(--color-border);border-radius:8px;background:var(--color-surface)">
      ...
      ${isCC ? `<p style="font-size:12px;margin:0 0 12px;color:var(--color-text-secondary)">Granted by ${vessel.customs_clearance_by || 'System'} at ${vessel.customs_clearance_at ? formatDateTime(vessel.customs_clearance_at) : 'Registration'}</p>` : ''}
      <div style="display:flex;gap:8px;margin-top:8px">
        ${isCC ? 
          `<button class="btn btn-danger btn-sm" onclick="submitClearance('${imo}', 'customs', false)">Deny</button>` : 
          `<button class="btn btn-success btn-sm" onclick="submitClearance('${imo}', 'customs', true)">Restore Grant</button>`
        }
      </div>
    </div>
...
window.submitClearance = async function(imo, type, granted) {
  ...
  if (!granted) {
    officer_id = prompt('Officer ID required for denial:');
    if (!officer_id) return; // Cancelled
  }
  ...
  if (res.success) {
    ...
    // Update local vessel state immediately without reloading everything
    const vessel = _vessels.find(v => v.imo === imo);
    if (vessel) {
    ...
    // update the table row badge (without a full table re-render)
    const row = document.getElementById(`row-${imo}`);
    if (row && vessel) {
      const isHC = vessel.health_clearance === 1;
      const isCC = vessel.customs_clearance === 1;
