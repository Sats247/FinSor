...
def init_db():
    ...
    if v_columns:  # Vessels table might be dynamically created from script
        if 'health_clearance' not in v_columns:
            conn.execute("ALTER TABLE vessels ADD COLUMN health_clearance INTEGER DEFAULT 1")
            conn.execute("ALTER TABLE vessels ADD COLUMN health_clearance_by TEXT")
            conn.execute("ALTER TABLE vessels ADD COLUMN health_clearance_at DATETIME")
            conn.execute("ALTER TABLE vessels ADD COLUMN customs_clearance INTEGER DEFAULT 1")
            conn.execute("ALTER TABLE vessels ADD COLUMN customs_clearance_by TEXT")
            conn.execute("ALTER TABLE vessels ADD COLUMN customs_clearance_at DATETIME")
            conn.commit()

        # Fix existing records
        conn.execute("UPDATE vessels SET health_clearance = 1 WHERE health_clearance IS NULL")
        conn.execute("UPDATE vessels SET customs_clearance = 1 WHERE customs_clearance IS NULL")
        conn.commit()
