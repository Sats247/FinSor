    ...
    timeout = setTimeout(async () => {
      ...
      if(res.success && res.data.length > 0) {
        ...
        dd.innerHTML = matches.map(m => `
          <div style="padding:10px; border-bottom:1px solid var(--color-border); cursor:pointer; background:var(--color-surface); transition:background 150ms;" onmouseover="this.style.background='var(--color-bg)'" onmouseout="this.style.background='var(--color-surface)'" onclick="linkFamily('${provId}', '${m.id}', this)">
      ...
      <div style="display:flex; flex-direction:column; gap:8px;">
        ${uniqueMatches.map(m => `
          <div style="background:var(--color-surface); padding:10px; border-radius:4px; border:1px solid var(--color-border); display:flex; justify-content:space-between; align-items:center;">
            ...
            <button class="btn btn-primary btn-sm" onclick="linkFamily('${provId}', '${m.id}', this)">Link as Family</button>
          </div>
        `).join('')}
      </div>
    `;
  ...
  openModal({
    ...
    body: `
      ...
      <div id="family-declare-section">
        ...
        <div style="display:flex;justify-content:space-between;align-items:center;margin-top:12px">
          <button class="btn btn-secondary btn-sm" onclick="addFamilyRow('${provId}')">+ Add Another</button>
          <button class="btn btn-primary btn-sm" onclick="submitFamily('${provId}', true)" id="btn-family-search">Search and Link</button>
        </div>
      </div>
      <div id="family-matches-section" style="display:none;margin-top:16px;padding:12px;background:var(--color-surface-hover);border-radius:6px;border:1px solid var(--color-border)">
      </div>
  ...
  if (res.success && res.data.length > 0) {
    container.innerHTML = res.data.map(m => `
      <div style="display:flex; justify-content:space-between; align-items:center; padding:10px 14px; border:1px solid var(--color-border); margin-bottom:8px; border-radius:4px; background:var(--color-surface)">
        ...
        <button class="btn btn-secondary btn-sm text-danger" onclick="unlinkFamily('${m.provisional_id}', '${provId}')">Unlink</button>
      </div>
    `).join('');
  } else {
...
window.unlinkFamily = async function(memberProvId, currentProvId) {
