...
def get_timeline(provisional_id):
    ...
    try:
        rows = db.execute(
            "SELECT stage, timestamp, updated_by FROM refugee_status_log WHERE provisional_id=? ORDER BY timestamp ASC",
            (provisional_id,)
        ).fetchall()
...
@refugee_bp.route('/<provisional_id>/appeal', methods=['POST'])
def create_appeal(provisional_id):
    data = request.get_json(silent=True) or {}
    ...
    try:
        ref = db.execute(
            "SELECT e.name FROM refugee_registrations rr JOIN entities e ON rr.entity_id=e.id WHERE provisional_id=?", 
            (provisional_id,)
        ).fetchone()
        if not ref:
            return api_error('Invalid Provisional ID', 404)
            
        db.execute(
            "INSERT INTO appeals (provisional_id, type, description) VALUES (?, ?, ?)",
            (provisional_id, a_type, desc)
        )
        # Write warning alert
        db.execute(
            "INSERT INTO alerts (type, message, severity, triggered_by) VALUES (?, ?, ?, ?)",
            ('appeal', f"Refugee {ref['name']} submitted an appeal: {a_type}", 'warning', 'Refugee Portal')
        )
        db.commit()
        return api_response(message='Appeal submitted successfully', status=201)
    finally:
        db.close()

@refugee_bp.route('/<provisional_id>/appeals', methods=['GET'])
def get_appeals(provisional_id):
    db = get_db()
    try:
        rows = db.execute(
