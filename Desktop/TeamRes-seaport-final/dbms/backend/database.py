...
def init_db():
    ...
    if v_columns:  # Vessels table might be dynamically created from script
        if 'health_clearance' not in v_columns:
            conn.execute("ALTER TABLE vessels ADD COLUMN health_clearance INTEGER DEFAULT 0")
            ...
            conn.execute("ALTER TABLE vessels ADD COLUMN customs_clearance INTEGER DEFAULT 0")
