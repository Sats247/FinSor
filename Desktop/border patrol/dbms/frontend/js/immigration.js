...
async function loadTravelers(q = '', status = '') {
  ...
  tbodyEl.innerHTML = items.map(r => {
    ...
    const normalizedStatus = isBlacklisted ? 'blacklisted' : (r.status === 'under_verification' ? 'under_verification' : 'active');
    ...
    return `<tr id="tr-traveler-${r.id}" style="${isBlacklisted ? 'background:#FEF2F2' : ''}">
